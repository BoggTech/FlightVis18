...
String fileAddress = "/Users/Administrator/Downloads/flights2k(1).csv";

ArrayList<FlightObject> flightObjects = new ArrayList<>();
float cancelledFlights;
float notCancelledFlights;
float[] flights;
...
void setup() {
  size(640, 640);
  read_in_the_file();
  float[] flights= {notCancelledFlights, cancelledFlights};
  thePieChart=new pieChart (flights, 200, 600, 600);
  thePieChart.setup();
}
...
void read_in_the_file() {
  ...
  // print("Please put the address of file of Flight Objects: ");
  try {
    FileReader fileReader = new FileReader("/Users/Administrator/Downloads/flights2k(1).csv");
  ...
  for (int i =0; i < flightObjects.size(); i++) {
    ...
    print(flightObjects.get(i).getCancelled() + ", ");
  }
  for (int i=0; i<flightObjects.size(); i++) {
    double temp =  flightObjects.get(i).getCancelled();
    if (temp==1) cancelledFlights++;
    if (temp==0) notCancelledFlights++;
  }
}
